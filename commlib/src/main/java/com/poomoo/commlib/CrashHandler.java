...
//            MainFragmentActivity.instance.finish();
    ...
    private boolean handleException(final Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                Looper.prepare();
                MyUtils.showToast(mContext, "我们的程序碰到了点小问题,请重新启动^_^");
                Looper.loop();
            }
        }.start();
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            File dir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/partTimeJob/");
            if (!dir.exists())
                dir.mkdirs();

            cmd = "chmod 777 " + Environment.getExternalStorageDirectory().getAbsolutePath() + "/partTimeJob/";
            Runtime.getRuntime().exec(cmd);
            FileOutputStream fos = new FileOutputStream(
                    Environment.getExternalStorageDirectory().getAbsolutePath() + "/partTimeJob/" + fileName);
            fos.write(sb.toString().getBytes());
            fos.close();
            return fileName;
        } catch (Exception e) {
            LogUtils.i("an error occured while writing file..." + e.getMessage());
        }
        return null;
    }
